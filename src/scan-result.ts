...
export function writeSummary(
  ...
  const matches: string[] = (result.failedPolicyMatches || []).map((pm) => {
    ...
    return `**${name}**: This image contains ${cveCount} ${severity} vulnerabilities${withFixes}, which is greater than the policy threshold (${packageCountThreshold})`;
  });

  const title =
    matches.length === 0
      ? `:heavy_check_mark: ${image} passed all policies`
      : `:heavy_multiplication_x: ${image} failed some policies`;
